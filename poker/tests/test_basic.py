...
def test_authentication(client, logged_in_user):
    rv = client.get('api/user/testauth',
                    headers={'x-access-token': logged_in_user['auth_token']})
    assert rv._status_code == 200
...
def test_authentication_fails_with_expired_token(client, logged_in_user):
...
@pytest.fixture
def holdem_2max_game(client, logged_in_user):
    rv = create_2max_holdem_game(client, logged_in_user['auth_token'])
    return rv.json


def test_create_game(client, logged_in_user):
    rv = create_2max_holdem_game(client, logged_in_user['auth_token'])
    print(rv.json)
    assert rv._status_code == 201